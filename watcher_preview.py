        ...
        # Linux is modified
            ...
            if rend_type == 'preview':
                ...
                if json_data['status'] == False:
                    ...
                    compress = True
                    chunk = False
                    logging.debug("Preview render service for {} requested".format(proj_id))
...
if __name__ == '__main__':
    ...
    log_file_name = os.path.join(
        ...
        Config.WATCHER_LOGS, 
        Config.PREVIEW_LOGS, 
        datetime.now().strftime("%Y.%m.%d-%H-%M-%S") + "_render_watcher_instance_preview.log"
    )
    ...
    logging.debug("This watcher is for preview render instances")
    w = Watcher()
    w.run()