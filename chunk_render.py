...
def get_chunk(user, send_end=None, compress_render=False, chunk_render=False, chunk_number=0, all_clips=True):
    try:     
        logging.debug('Removing chunk files...')   
        # Removes chunk files if they exist
        if chunk_render:
            chunk_del_dir = os.path.join(attach_dir, user)
            for _,_, files in os.walk(chunk_del_dir):
                for f in files:
                    if f.startswith(user + "_com_chunk"):
                        logging.debug('{} removed'.format(f))
                        os.remove(os.path.join(chunk_del_dir, f))
        ...
        if not chunk_render:
            if os.path.exists(os.path.join(attach_dir, user, 'Template.json')):
                ...
                logging.debug("Intro json data can't be found, continuing render service")
        ...
        logging.error("Unknown Fatal Error occured during render instance of '{}'".format(user))
        logging.exception("")
        logging.error("Exiting program without writing video file correctly")
        results = "Video not rendered [ERROR OCCURED, VIEW LOGS '{}' FOR MORE DETAILS]".format(log_name), 99
        if send_end is not None:
