        ...
        # Linux is modified
            ...
            if rend_type == 'chunk':
                ...
                if json_data['status'] == False:
                    ...
                    compress = False
                    chunk = True
                    logging.debug("Chunk render instance for {} requested".format(proj_id))
...
if __name__ == '__main__':
    ...
    log_file_name = os.path.join(
        ...
        Config.WATCHER_LOGS, 
        Config.CHUNK_LOGS, 
        datetime.now().strftime("%Y.%m.%d-%H-%M-%S") + "_render_watcher_instance_chunk.log"
    )
    ...
    logging.debug("This watcher is for chunk render requests")
    w = Watcher()
    w.run()